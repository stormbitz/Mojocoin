...
const string strMessageMagic = "Mojocoin Signed Message:\n";
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (nActualSpacing < 1)
        nActualSpacing = 1;
	if (nActualSpacing > nTargetSpacing * 3)
		nActualSpacing = nTargetSpacing * 3;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Mojocoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        if (!fTestNet)
        {
            txNew.nTime = 1455033871;
        }
        else
        {
            txNew.nTime    = 1455033872; // Without separate txNew.nTime near block.nTime, CheckBlock fails on coinbase timestamp for TestNet created on 14042015
        }
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1455033877;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1004377;
        if (fTestNet)
        {
            block.nTime    = 0;
            block.nNonce   = 0;
        ...
        // debug print
        assert(block.hashMerkleRoot == uint256("0xde5b0282c1c5af1cfd8f3158b6c436f004e582a24bb2fa0e40b14186208eddc3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe1, 0xee, 0xb2, 0xd4 };
